...
<script>
  ...
  import {noClassSelectStudentList} from '@/api/teach'
  // 表单字段
  ...
  export default {
    ...
    methods: {
      ...
      getMock(courseId) {
        ...
        let params = {}
        params.search = courseId
        noClassSelectStudentList(params).then((response)=>{
          for(let data of response.result){
            let d = {};
            d.title = data.studentName;
            d.key = data.id.toString();
            d.description = data.studentName
            mockData.push(d);
          }
        })
